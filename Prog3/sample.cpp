...
MjbSphere( float radius, int slices, int stacks )
{
	...
	for( int ilat = 0; ilat < NumLats; ilat++ )
	{
		...
		for( int ilng = 0; ilng < NumLngs; ilng++ )
		{
			...
			if( Distort )
			{
                p->s =  ((lng + M_PI) / sin(texture_animation*(2.*M_PI)));
                p->t =  ((lat + M_PI / 2.) / cos(texture_animation * (M_PI*2.)));
			}
			else
			{
...
InitGraphics( )
{
    ...
    texture2 = BmpToTexture("worldtex.bmp", &texture_w2, &texture_h2);
