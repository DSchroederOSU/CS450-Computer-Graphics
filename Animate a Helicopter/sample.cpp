...
//for animation
float Time;
#define MS_IN_THE_ANIMATION_CYCLE	7000
float BladeAngle;
...
Display( )
{
    ...
    {
        gluLookAt( 15., 7., -1.,     0., 0., 0.,     0., 1., 0. );
    }
    else
    {
        gluLookAt( -0.4, 1.8, -4.9,     0., 1., -10.,     0., 1., 0. );
    }
	...
	glPushMatrix();
		...
		glRotatef(90, 1, 0, 0.);
		glRotatef(-(GLfloat)360. * BladeAngle, 0., 0., 1.);
		glCallList(blade);
	glPopMatrix();
	...
	glPushMatrix();
		...
		glRotatef(90, 0, 1., 0.);
		glRotatef(90, 0, 0., 1.);
		glRotatef(-(GLfloat)360. * (3 * BladeAngle), 0., 0., 1.);
...
InitLists( )
{
    ...
    //1.5 and be attached at: (.5,2.5,9.).
    ...
    glNewList( backblade, GL_COMPILE );
        ...
        glBegin( GL_TRIANGLES );
        glVertex2f(  SMALL_BLADE_RADIUS, BLADE_WIDTH/2. );
        glVertex2f( 0. , 0. );
        glVertex2f(  SMALL_BLADE_RADIUS,  -BLADE_WIDTH/2. );
    ...
    distanceObj = glGenLists( 1 );
	int height = 3;
		int radius = 2;
		const float DEG2RAD = 3.14159/180;
        glNewList( distanceObj, GL_COMPILE );
        glColor3f( 1, 1, 0 );
		int j;
